...
import { Subscription, debounceTime, fromEvent, take, timer } from 'rxjs';
...
export class TocComponent implements OnInit, OnDestroy, OnChanges {
  @Input()
  contentReference: HTMLElement | undefined;
  ...
  tocWrapper: ElementRef<HTMLElement> | undefined;
  ...
  private maxTopOffset: number | undefined;
  ...
  lookupHeadings() {
    ...
    const removeAnchor = (text: string) => {
      ...
      return Number(anchorId) >= 0 ? text.slice(0, anchorId as number) : text;
    };
  ...
  checkViewportBoundaries() {
    this.isPositionFixed = (this.maxTopOffset as number) > window.pageYOffset;
    const { nativeElement } = this.tocWrapper as ElementRef<HTMLElement>;
    ...
    if (!this.isPositionFixed) {
      ...
      this.renderer.setStyle(
        nativeElement,
        'top',
        (this.maxTopOffset as number) + initialTopOffset + 'px'
      );
